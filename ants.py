...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        "*** YOUR CODE HERE ***"
        if exit:
            # we have a place called exit which acts as an enterance 
            exit.entrance = self
        # END Problem 2
...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            currentAnt = place.ant
            incomingAnt = self

            if currentAnt.can_contain(incomingAnt):
                containAnt = currentAnt
                containeeAnt = incomingAnt
            elif incomingAnt.can_contain(currentAnt):
                containAnt = incomingAnt
                containeeAnt = currentAnt
            else:
                assert place.ant is None, 'Two ants in {0}'.format(place)
            
            place.ant = containAnt
            containAnt.store_ant(containeeAnt)
            # END Problem 8b
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        This method returns None if there is no such Bee (orb none in range).
        """
        # BEGIN Problem 3 and 4
        distance = 0
        currentPlace = self.place
        while (not currentPlace.is_hive) :
            if len(currentPlace.bees) and self.lower_bound <= distance <= self.upper_bound:
...
# throw at closest bee within range
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        # you can't do self.reduce because you will keep overriding the current method
        # does damage when recieve damage
        
        newPlace = self.place
        super().reduce_health(amount)
        for bee in newPlace.bees[:]:
            if self.health <= 0:
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        ...
        # available to chew
            ...
            # if there is a bee in place
            #     we chew that beep
            #     health of bee = 0
            #     set chewing to chewing turns
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        containedAnt = self.ant_contained
        if containedAnt:
            containedAnt.action(gamestate)

        # if there is an ant:
        #     we want to execute the gamestate of that ant
        # END Problem 8a
